...
//领域服务：主要的业务规则编写
import (
	pubAdapter "user_context/rhombic/acl/adapters/publishers"
	repoAdapter "user_context/rhombic/acl/adapters/repositories"
...
func (service *Service)Registered(entity entity.Entity) (ok bool) {
	if ok = service.Repository.CheckIsExist(entity); ok {return !ok} 	// 账户存在，返回注册失败
	if err := service.Repository.Insert(entity);err != nil {return} 	// 账户写入失败，返回注册失败
	if ok = service.Publisher.Registered(entity.ID, entity.Event); !ok {return ok} // 注册事件失败， 返回注册失败
	return true
}
